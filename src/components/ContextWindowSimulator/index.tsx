...
const SCRIPT: Array<{
  ...
  assistant: { text: string; tokens: number };
}> = [
  {
    user: { text: 'What is a context window in an LLM?', tokens: 12 },
    assistant: {
      text: 'The context window is the maximum number of tokens the model can process in a single request — your system prompt, the full conversation history, and the new user message all count against it. Typical sizes today are 8K, 32K, or 128K tokens.',
      tokens: 520,
    },
  },
  {
    user: {
      text: 'Do older messages get forgotten between turns?',
      tokens: 14,
    },
    assistant: {
      text: 'LLMs have no memory between API calls. Every turn, the client re-sends the entire prior conversation so the model can “see” the full thread again. Once the combined input exceeds the window, the oldest messages must be dropped.',
      tokens: 640,
    },
  },
  {
    ...
    assistant: {
      text: 'Each new turn re-processes every earlier token through the prefill phase. More input tokens mean more compute, higher time-to-first-token, and a larger KV cache, so latency climbs as the chat grows.',
      tokens: 700,
    },
  },
  {
    ...
    assistant: {
      text: 'Summarize or drop old turns before re-sending, use prefix caching so the prior KV is not recomputed, or move to a model with a larger window, though larger windows cost more per request.',
      tokens: 580,
    },
  },
  {
    ...
    assistant: {
      text: 'Most APIs either return an error or silently truncate the oldest tokens. Either way, earlier context is lost, so the model may contradict itself or forget facts it previously agreed on.',
      tokens: 520,
    },
  },
  {
    ...
    assistant: {
      text: 'Not always. Larger windows raise prefill latency and cost, and models often attend less accurately to content in the middle of very long inputs. Right-size the window to the workload.',
      tokens: 600,
    },
  },
  {
    ...
    assistant: {
      text: 'Streaming only affects how the output is delivered. The prefill cost of a long input is unchanged, so you still pay full TTFT for every token in the prompt before the first output token appears.',
      tokens: 560,
    },
  },
  {
    user: {
      text: 'Can KV cache offloading help with long context?',
      tokens: 13,
    },
    assistant: {
      text: 'Yes. Offloading spills cold KV blocks from GPU to CPU or disk, trading a small retrieval latency for much more concurrency on long-context requests — useful when GPU memory is the bottleneck.',
      tokens: 620,
    },
  },
];
  ...
  function addTurn() {
    ...
    setTurns((ts) => [
      ...
      },
      {
        id: id2,
        role: 'assistant',
        tokens: round.assistant.tokens,
        text: round.assistant.text,
        roundIdx: newRoundIdx,
      },
    ]);
  }
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.headerDesc}>
          Every turn, the full conversation is re-sent to the model. Watch the
          window fill up.
        </div>
      </div>
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.convoCol}>
            <div className={styles.colLabel}>
              Input to the model{' '}
              {latestRound > 0 ? `— round ${latestRound}` : ''}
            </div>
