...
export default function RequestLifecycle() {
  ...
  function stageNumClass(st: ReturnType<typeof stageStatus>) {
    return st === 'active'
      ? styles.stageNumActive
      : st === 'done'
        ? styles.stageNumDone
        : styles.stageNumInactive;
  }
            ...
            return (
              <React.Fragment key={i}>
                <div className={`${styles.stage} ${stageClass(st)}`}>
                  <div className={`${styles.stageNum} ${stageNumClass(st)}`}>
                    {i + 1}
                  </div>
                ...
                {i < STAGE_LABELS.length - 1 && (
                  <div
                    className={`${styles.arrow} ${arrowClass(arrowStatus(i))}`}
                  >
                    →
                  </div>
                )}
        ...
        <div className={styles.detail}>
          {phase === 'idle' && (
            <div className={styles.idleMsg}>
              Press <strong>Play</strong> to send the prompt through the
              inference pipeline.
            </div>
          )}
          ...
          {phase === 'tokenizing' && (
            <div className={styles.detailContent}>
              ...
              <div className={styles.detailDesc}>
                The tokenizer splits raw text into subword units and maps them
                to integer IDs the model understands.
              </div>
              ...
              <div className={styles.counter}>
                {tokensShown} / {INPUT_TOKENS.length} tokens
              </div>
            </div>
          )}
          ...
          {phase === 'serving' && (
            <div className={styles.detailContent}>
              ...
              <div className={styles.detailDesc}>
                The server receives the tokenized request, allocates a KV cache
                block, and enqueues it for the next prefill batch.
              </div>
          ...
          {phase === 'prefill' && (
            <div className={styles.detailContent}>
              ...
              <div className={styles.detailDesc}>
                All {INPUT_TOKENS.length} input tokens are processed in one
                parallel forward pass. Attention keys and values are written to
                the KV cache.
              </div>
              <div className={styles.prefillRow}>
                {INPUT_TOKENS.map((tok, i) => (
                  <span
                    key={i}
                    className={`${styles.chip} ${styles.chipPrefill}`}
                  >
                    {tok.trim() === '' ? '·' : tok}
                  </span>
                ))}
          ...
          {(phase === 'decoding' || phase === 'done') && (
            <div className={styles.detailContent}>
              ...
              <div className={styles.detailDesc}>
                One token per forward pass, streamed back to the client as soon
                as it is produced.
              </div>
